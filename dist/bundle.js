...
function newGame() {
	...
	b.state.forEach(function (cell, index) {
		html_cells[index].addEventListener('click', function () {
			// If cell is already occupied or the board is in a terminal state or it's not human's turn, return false
			...
			// If it's a terminal move
			if (b.isTerminal()) {
				var winner = b.isTerminal().winner;
				if (winner !== 'draw') {
